...
class g1Sim(MuJoCoBase):
  ...
  def reset(self):
    ...
    # self.data.qpos[7] = -math.pi / 8
    # self.data.qpos[10] = math.pi / 4
    # self.data.qpos[11] = -math.pi / 8
    # self.data.qpos[13] = -math.pi / 8
    # self.data.qpos[16] = math.pi / 4
    # self.data.qpos[17] = -math.pi / 8
  ...
  def simulate(self):
    while not glfw.window_should_close(self.window):
      ...
      while (self.data.time - simstart <= 1.0/200.0 and not self.pause_flag):
        ...
        qp = self.data.qpos[7:].copy()
        # print(len(self.data.qpos),len(qp))
        qv = self.data.qvel[6:].copy()
        # print(len(self.data.qvel),len(qv))
...
def main():
    # ros init
    rospy.init_node('nao_sim', anonymous=True)

    # get xml path
    rospack = rospkg.RosPack()
    rospack.list()
    g1_desc_path = rospack.get_path('nao_description')
    xml_path = g1_desc_path + "/scene.xml"
    sim = g1Sim(xml_path)
    sim.reset()
    sim.simulate()
